    ...
    >
      {filteredData.map((data, index) => (
        <Link href={`/portfolios/${data.link}`} key={`${data.category}-${data.name}-${index}`}>
          <motion.div
            className="relative group rounded-xl shadow-lg overflow-hidden bg-gradient-to-br from-gray-100 to-gray-50 hover:from-blue-50 hover:to-blue-100 transition-all duration-300"
            variants={{
          ...
          >
            ...
            <div className="p-6 space-y-3">
              <h2 className="text-2xl font-semibold text-gray-800 group-hover:text-blue-600 transition-colors duration-300">
                {data.name}
              </h2> {/* Changed from <h3> to <h2> */}
              <p className="text-gray-600 line-clamp-3">
                {data.details[0]} {/* Display the first detail */}
              </p>
              {data.category && (
            ...
            {/* Card Flip Back */}
            <motion.div
              className="absolute inset-0 bg-blue-600 text-white p-6 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-center items-center"
              initial={{ rotateY: 180 }}
              animate={{ rotateY: 0 }}
              exit={{ rotateY: -180 }}
            >
