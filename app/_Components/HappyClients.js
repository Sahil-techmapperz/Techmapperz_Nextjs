...
const HappyClients = () => {
  return (
    <section className="relative py-16 bg-gray-900">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 px-6 max-w-5xl mx-auto">
        {Clients.map((client, index) => (
          <div
            key={index}
          ...
          >
            <Image
              className="object-contain grayscale group-hover:grayscale-0 transition-all duration-500 ease-in-out transform group-hover:scale-105"
              src={client}
              alt={`Client logo ${index + 1}`}
            />
          </div>
